...
**28 fichiers de recette.** Ce nombre est le dénominateur de « suite complète
...
| `tests/effectif-v2.spec.js` | la phase 1 du pivot v2 (concept §2, décision 77) : onze dès la manche 1 tiré avec ses familles, l'IA à onze aussi, plus de montée 5→11, effectif 15 (banc 4), achat à effectif plein = volet de remplacement, jamais sous onze |
...
| Poids à l'ouverture, PIRE tirage, densité 1 (`da.spec.js`) | **1 653 Ko** (socle 1 281 + les 5 key arts les plus lourds 372 — 1 580 → 1 631 → 1 643 → 1 653 : refonte +41 Ko, synergies +8, étape 4 +12, phase 1 v2 +10) | **≤ 1 500 Ko** | la livraison qui différera `match-scene.js` — attention : 1 653 − 130 = **1 523**, le différé seul ne suffit plus, il faudra raboter les Ko restants |
...
| L'art des six promus du onze de départ (`portraits.spec.js`, dette signalée en console) | **Gilbert, Norbert, Fernand, Marius, Lucien, Célestin** jouent en silhouette neutre + glyphe de poste (le repli déclaré, règle M4) | leurs visuels S6-S11 (carte + frontale + figurine), même fournée que S1-S5 | la prochaine fournée d'art de Gabriel — la recette exige déjà l'intégrité de toute entrée S6-S11 qui apparaîtra dans la table |
